...
var $author$project$PhotoGroove$rangeSlider = F2(
	function (attributes, children) {
		return $mdgriffith$elm_ui$Element$html(
			A3($elm$html$Html$node, 'range-slider', attributes, children));
	});
...
var $author$project$PhotoGroove$viewSizeChooser = function (model) {
	return A2(
			...
			]),
		{
			label: A2(
				...
				_List_Nil,
				$mdgriffith$elm_ui$Element$text('Thumnail Size:')),
			onChange: function (_new) {
				return $author$project$PhotoGroove$ClickedSize(
					{selected: true, size: _new});
			},
			options: A2(
				$elm$core$List$map,
				function (size) {
					return A2(
						...
						$mdgriffith$elm_ui$Element$text(
							$author$project$PhotoGroove$sizeToClass(size.size)));
				},
				model.sizes),
			selected: $elm$core$Maybe$Just(model.chosenSize)
		});
};
	...
	function (isSelected, choosenSize, thumb) {
		return A2(
				...
				A2($author$project$PhotoGroove$selectedImageEffect, thumb.url, isSelected)),
			{
				...
				src: _Utils_ap($author$project$PhotoGroove$urlPrefix, thumb.url)
			});
	});
							...
							[
								...
								A2(
										...
										]),
									{
										...
										onPress: $elm$core$Maybe$Just($author$project$PhotoGroove$ClickedSurpireseMe)
									}))
							]))
					])),
				A2(
